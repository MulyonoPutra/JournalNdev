...
public class ProductController {
  ...
  public ResponseEntity<Product> getByName(@PathVariable("name") String name) {
    if (!productService.existsByName(name)) {
      return new ResponseEntity(
        new Messages("Product not found!"),
        HttpStatus.NOT_FOUND
      );
    }
  ...
  public ResponseEntity<?> create(@RequestBody ProductDTO productDto) {
    if (StringUtils.isBlank(productDto.getName())) return new ResponseEntity<>(
      new Messages("BAD REQUEST"),
      HttpStatus.BAD_REQUEST
    );
    if (productDto.getPrice() < 0) return new ResponseEntity<>(
      new Messages("Product must be not null"),
      HttpStatus.BAD_REQUEST
    );
    if (
      productService.existsByName(productDto.getName())
    ) return new ResponseEntity<>(
      new Messages("Product already exist!"),
      HttpStatus.BAD_REQUEST
    );
  ...
  public ResponseEntity<?> update(
    ...
    if (!productService.existsById(id)) return new ResponseEntity<>(
      new Messages("ID not found!"),
      HttpStatus.NOT_FOUND
    );
    if (
